...
## - STRATIFIED 80/20 split (PERSON/ID-level; no leakage across intervals)
## - Saves per-seed checkpoints: checkpoints_base_observed_IDsplit/seed_###_IDsplit.pt
## - Saves a SMALL “slim” RData for later eval scripts:
##     TRAINING_base_observed_IDsplit_100seeds_slim.RData
...
## ================================================================
ckpt_dir <- "checkpoints_base_observed_IDsplit"
...
## ID index for random effect (global across all rows)
...
## ================================================================
df_core <- data_surv_al[, c("id", event_var, predictors)]
...
## id vector aligned to df_core
id_core <- as.character(df_core$id)
...
## ================================================================
## 6) Stratified train/test split (PERSON/ID-level)
##    - No person appears in both train and test
##    - Stratification is on PERSON-level event (ever died across intervals)
## ================================================================
set.seed(123)
...
person_df <- data.frame(
  ...
  group_by(id) %>%
  summarise(
    y_person = as.integer(any(y_interval == 2L)),  # 1 if ever event
    .groups = "drop"
  )

id_tr <- caret::createDataPartition(person_df$y_person, p = 0.80, list = FALSE)
...
cat("Train/Test (interval rows):", nrow(Xtr), nrow(Xte), "\n")
cat("Train/Test (unique persons):", length(unique(id_core[tr])), length(unique(id_core[te])), "\n")
cat("Class counts (train intervals):", paste(table(factor(ytr, levels = 1:2)), collapse=" "), "\n")
cat("Class counts (test intervals): ", paste(table(factor(yte, levels = 1:2)), collapse=" "), "\n")
cat("Person-level event rate (train):", mean(person_df$y_person[person_df$id %in% train_ids]), "\n")
cat("Person-level event rate (test): ", mean(person_df$y_person[person_df$id %in% test_ids ]), "\n")
...
## ================================================================
## 10) Seed runner: train-or-load checkpoint, then evaluate TEST
## ================================================================
run_baseline_seed <- function(torch_seed,
  ...
  ckpt_path <- file.path(ckpt_dir, sprintf("seed_%03d_IDsplit.pt", torch_seed))
...
save(
  results_slim,
  keep_rows, tr, te,
  train_ids, test_ids, person_df,
  predictors, classes,
  cw,
  static_idx, tv_idx, n_steps, tv_step_feat_dim,
  id_index_core,
  id_core,
  file = "TRAINING_base_observed_IDsplit_100seeds_slim.RData"
)

cat("\nSaved: TRAINING_base_observed_IDsplit_100seeds_slim.RData\n")
