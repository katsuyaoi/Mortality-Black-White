...
## -------------------------
ckpt_dir <- "checkpoints_fullspec_IDsplit"
...
## ================================================================
## ================================================================
## 5. Stratified train/test split (ID-level / person-level)
##    - No person appears in both train and test
##    - Stratification is on PERSON-level event (ever died across intervals)
## ================================================================
set.seed(123)

# PERSON-level label: ever-event across that person's intervals
# (works for discrete-time hazard format where died_ can appear once)
person_df <- data.frame(
  ...
  dplyr::group_by(id) %>%
  dplyr::summarise(
    y_person = as.integer(any(y_interval == 2L)),  # 1 if ever class "1", else 0
    .groups = "drop"
  )

# stratified split on person outcome
...
# map back to ROW indices (intervals) by ID membership
...
cat("Class counts (train intervals):",
    paste(table(factor(ytr, levels = seq_along(classes))), collapse = " "), "\n")
cat("Class counts (test intervals): ",
...
## ================================================================
save.image("TRAINING_fullspec_IDsplit_100seeds.RData")
cat("\nSaved end-of-training image: TRAINING_fullspec_IDsplit_100seeds.RData\n")
